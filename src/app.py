...
def run_react_agent(user_query: str, provider, mcp_server: MCPAcademicServer) -> list:
    ...
    while step < MAX_ITERATIONS:
        ...
        # Trường hợp 2: LLM đề xuất gọi Tool (Action)
            ...
            # Nạp Observation vào lượt kế tiếp để LLM tiếp tục suy luận hoặc trả lời.
            current_prompt = (
                f"Câu hỏi ban đầu: {user_query}\n"
                f"Kết quả Observation từ tool {tool_name}: {obs_str}\n"
                "Hãy tiếp tục suy luận theo yêu cầu ban đầu. Nếu đã đủ dữ liệu, trả lời bằng văn bản."
            )
            print("🧠 [Thought]: Đã nạp Observation. Tiếp tục vòng lặp ReAct.")
